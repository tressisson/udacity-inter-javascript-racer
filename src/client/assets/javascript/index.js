...
async function handleCreateRace() {
	try {
		...
		//store.race_id = race.id - 1;
			...
			alert("you have not selected a racer and a track")
		}
	} catch(err) {
...
async function handleCreateRace() {
	...
	if (store.player_id && store.track_id) {
		...
		alert("you haven't selected a player and a track");
	}
}
...
function runRace(raceID) {
	return new Promise(resolve => {
		...
		try {
			...
			console.log(error, "Problem occured running runRace()")
		}
...
function getTracks() {
	return fetch(`${SERVER}/api/tracks`)
		.then(res => res.json())
		.catch(err => console.log('getTracks error', err));
}
...
function getRacers() {
	...
	return fetch(`${SERVER}/api/cars`)
		.then(res => res.json())
		.catch(err => console.log('getRacers error', err));
}
